...
public interface GrammarProgressMapper {
    ...
    @Update("UPDATE grammar_progress SET is_learned_theory = #{is_learned_theory}, is_finish_quiz = #{is_finish_quiz}, last_modified = GETDATE(), is_deleted = #{is_deleted} " +
    ...
    @Update("UPDATE grammar_progress SET is_finish_quiz = 1, last_modified = GETDATE() WHERE user_id = #{user_id} AND grammar_id = #{grammar_id} AND course_id = #{course_id} AND is_deleted = 0")
    ...
    @Select("SELECT CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END FROM grammar_progress WHERE user_id = #{user_id} AND grammar_id = #{grammar_id} AND course_id = #{course_id} AND is_deleted = 0")
