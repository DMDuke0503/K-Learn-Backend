...
public interface GrammarProgressMapper {
    ...
    @Insert("INSERT INTO grammar_progress (user_id, grammar_id, course_id, is_learned_theory, is_finish_quiz, is_failed_quiz, last_modified, is_deleted) "
            + "VALUES (#{id.user_id}, #{id.grammar_id}, #{id.course_id}, #{is_learned_theory}, #{is_finish_quiz}, #{is_failed_quiz}, #{last_modified}, #{is_deleted})")
    ...
    @Results({
            ...
            @Result(property = "is_failed_quiz", column = "is_failed_quiz"),
    ...
    // Soft delete a GrammarProgress entry by setting is_deleted to 1
    @Update("UPDATE grammar_progress SET is_deleted = 1, last_modified = GETDATE() WHERE user_id = #{user_id} AND grammar_id = #{grammar_id} AND course_id = #{course_id}")
    ...
    @Select("SELECT COUNT(*) FROM grammar_progress WHERE user_id = #{user_id} AND course_id = #{course_id} AND is_learned_theory = 1 AND is_finish_quiz = 1 AND is_deleted = 0")
    ...
    @Select("SELECT COUNT(*) FROM grammar_progress WHERE user_id = #{user_id} AND course_id = #{course_id} AND (is_learned_theory = 0 OR is_finish_quiz = 0) AND is_deleted = 0")
    int countNotLearnedGrammar(@Param("user_id") Integer userId, @Param("course_id") Integer courseId);

    // Mark a grammar entry as learned
    @Update("UPDATE grammar_progress SET is_learned_theory = 1, last_modified = GETDATE() WHERE user_id = #{user_id} AND grammar_id = #{grammar_id} AND course_id = #{course_id} AND is_deleted = 0")
    ...
    @Update("UPDATE grammar_progress SET is_finish_quiz = 1, is_failed_quiz = 0, last_modified = GETDATE() WHERE user_id = #{user_id} AND grammar_id = #{grammar_id} AND course_id = #{course_id} AND is_deleted = 0")
    ...
    @Update("UPDATE grammar_progress SET is_failed_quiz = 1, last_modified = GETDATE() WHERE user_id = #{user_id} AND grammar_id = #{grammar_id} AND course_id = #{course_id} AND is_deleted = 0")
    void markGrammarQuizAsFailed(@Param("user_id") Integer userId, @Param("grammar_id") Integer grammarId,
            @Param("course_id") Integer courseId);

    // Check if a GrammarProgress entry exists for a user, grammar, and course
    @Select("SELECT CASE WHEN EXISTS (SELECT 1 FROM grammar_progress WHERE user_id = #{user_id} AND grammar_id = #{grammar_id} AND course_id = #{course_id} AND is_deleted = 0) THEN 1 ELSE 0 END")
