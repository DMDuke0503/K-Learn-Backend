...
public interface ComprehensiveTestResultsMapper {
    ...
    // Get the most recent comprehensive test result by course_id, user_id, and test_type, including course and user data
    @Select("SELECT TOP 1 ctr.*, c.*, u.* FROM comprehensive_test_results ctr " +
            "JOIN courses c ON ctr.course_id = c.id " +
            "JOIN users u ON ctr.user_id = u.id " +
            "WHERE ctr.course_id = #{courseId} AND ctr.user_id = #{userId} AND ctr.test_type = #{testType} AND ctr.is_deleted = 0 " +
